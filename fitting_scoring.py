...
# ---------------------------------------------------------------------------------------------
def evaluate_population(model, instructions, data, population, exportfile):
    ...
    trackR2PredTest, trackRMSE, trackMAE, trackAcceptPred, trackCoefficients = InitializeTracks()
    ...
    for i in range(numOfPop):
        ...
        model_rmse, num_acceptable_preds = calculateRMSE(TestY, Yhat_testing)
        model_mae = calculateMAE(TestY, Yhat_testing)

        # Calculating fitness value
        if 'dim_limit' in instructions:
            fitness[i] = get_fitness(xi, TrainY, ValidateY, Yhat_training, Yhat_validation, dim_limit=instructions['dim_limit'])
        else:
            fitness[i] = get_fitness(xi, TrainY, ValidateY, Yhat_training, Yhat_validation)
        ...
        if fitness[i] < unfit:
            ...
            trackFitness[idx] = sigfig(fitness[i])
            trackModel[idx] = instructions['algorithm'] + ' with ' + instructions['MLM_type']
            trackDimen[idx] = int(xi.__len__())

            trackR2[idx] = sigfig(r2_train)
            trackR2PredValidation[idx] = sigfig(r2validation)
            trackR2PredTest[idx] = sigfig(r2test)

            trackRMSE[idx] = sigfig(model_rmse)
            trackMAE[idx] = sigfig(model_mae)
            trackAcceptPred[idx] = sigfig(float(num_acceptable_preds) / float(Yhat_testing.shape[0]))

        # For loop ends here.

    write(exportfile, trackDesc, trackFitness, trackModel, trackDimen, trackR2, trackR2PredValidation, trackR2PredTest, trackRMSE, trackMAE, trackAcceptPred)
...
# ---------------------------------------------------------------------------------------------
def get_fitness(xi, T_actual, V_actual, T_pred, V_pred, gamma=3, dim_limit=None, penalty=0.05):
    n = len(xi)
    mT = len(T_actual)
    mV = len(V_actual)
...
# ---------------------------------------------------------------------------------------------
def write(exportfile, descriptors, fitnesses, modelnames,
